...
const tokens: Token[] = [
  {
    tokenIcon: "/crypto-icons/eth.svg",
    name: "e USD",
    balance: 123567.1,
    usdValue: 10000.0,
    priceFeed: 0.33,
  ...
  {
    tokenIcon: "/crypto-icons/strk.svg",
    name: "STRK",
    balance: 12987.556,
    usdValue: 1521.0,
    priceFeed: 0.52,
  ...
  {
    tokenIcon: "/crypto-icons/btc.svg",
    name: "BTC",
    balance: 0,
    usdValue: 0.0,
    priceFeed: 100987.25,
  ...
  {
    tokenIcon: "/crypto-icons/solana.svg",
    name: "SOL",
    balance: 0,
    usdValue: 0.0,
    priceFeed: 206.01,
  ...
  {
    tokenIcon: "/crypto-icons/ethereum.svg",
    name: "ETH",
    balance: 0.000312,
    usdValue: 500,
    priceFeed: 3626.86,
  ...
  {
    tokenIcon: "/crypto-icons/crypto.svg",
    name: "WLD",
    balance: 0,
    usdValue: 0.0,
    priceFeed: 2.36,
...
const SendModal = () => {
    ...
    return (
      <div className="w-full h-full ">
        <div className="w-full flex justify-between items-center my-5">
          <p className="font-medium text-sm">Please select the Token you want to send</p>
          <div className="flex gap-x-1 items-center">
            <Plus color="#767676" size={9} className="cursor-pointer" onClick={() => {}} />
            <p className="font-medium text-sm text-[#767676]">Select another account</p>
          </div>
        </div>
  ...
  const EnterDetails = ({ selectedToken, onNext }: EnterDetailsProps) => {
    ...
    return (
      <div className="w-full h-full mt-[3rem]">
        <div className="border h-[3.75rem] full rounded-md flex items-center justify-between p-3 bg-[#FCF8FF] border-[#F7ECFF] cursor-pointer transition-allduration-300">
          <div className="flex items-center gap-x-5">
            ...
            <div className="flex flex-col ">
              ...
              <p className="text-base font-medium">${selectedToken.priceFeed.toLocaleString()}</p>
            </div>
          </div>

          <div className="flex flex-col">
            <p className="font-medium text-sm">{selectedToken.balance.toLocaleString()}</p>
            <p className="font-normal text-[0.625rem]">${selectedToken.usdValue.toLocaleString()}</p>
          </div>
        </div>
        <div className="my-4">
          <label className="block text-sm font-medium">Enter valid wallet address</label>
          <input
            type="text"
        ...
        <button className="w-full bg-purple-600 text-white py-2 rounded-lg" onClick={handleSubmit}>
          Send
        </button>
      </div>
    )
  ...
  const Confirmation = ({ transactionDetails, selectedToken, onReset }: ConfirmTransactionProps) => {
    ...
    return (
      <div className="text-center">
        ...
        <p className="text-xl font-normal text-gray-500">
          You have successfully sent ${transactionDetails.amount} {selectedToken?.name} to{" "}
          <span className="font-medium">{formatWalletAddress(transactionDetails.walletAddress)}</span>
        </p>

        <p className="text-[#666666] font-normal text-xs">
          Transfer request is being processed and user should receive money in a few minutes..😎
        </p>
  ...
  const TokenDisplay = ({ token, onClick, isSelected }: TokenDisplayProps) => {
    return (
      <div
        className={`border h-[3.75rem] w-[15.75rem] rounded-md flex items-center justify-between p-3 hover:bg-[#F7ECFF] cursor-pointer transition-all duration-300 ${
          isSelected ? "bg-[#F7ECFF] border-[#D1C4E9]" : "border-[#F7ECFF] hover:bg-[#F7ECFF]"
        }`}
      ...
      >
        <div className="flex items-center gap-x-5">
          ...
          <div className="flex flex-col ">
            ...
            <p className="text-base font-medium">${token.priceFeed.toLocaleString()}</p>
          </div>
        </div>

        <div className="flex flex-col">
          <p className="font-medium text-sm">{token.balance.toLocaleString()}</p>
          <p className="font-normal text-[0.625rem]">${token.usdValue.toLocaleString()}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center ">
      ...
      >
        {step === 1 || step === 3 ? (
          <X size={18.75} color="#000000" className="absolute top-5 left-8 cursor-pointer" onClick={() => {}} />
        ) : (
          <MoveLeftIcon
            size={18.75}
            color="#000000"
        ...
        {step !== 3 && (
          <>
            <h2 className="text-2xl font-semibold mb-[1.7rem] text-center mt-[0.6rem]">Send Money</h2>

            <div className="w-fit mx-auto">
              <p className="text-xs font-medium w-fit text-[#666666]">Total Balance</p>
              <div className="flex items-end gap-x-6 w-fit">
                <p className="font-medium text-xl">${(11567).toLocaleString()}</p>
        ...
        {step === 2 && <EnterDetails selectedToken={selectedToken!} onNext={handleNext} />}
        {step === 3 && (
          <Confirmation transactionDetails={transactionDetails!} selectedToken={selectedToken!} onReset={handleReset} />
        )}
      </div>
    </div>
  )
